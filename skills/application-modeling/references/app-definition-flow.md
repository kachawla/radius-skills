...
3. Inspect workload files for explicit port configuration such as `EXPOSE`, `listen` directives, or server startup args.
4. Inspect the application code for resource expectations such as PostgreSQL, blob storage, or AI agent endpoint usage.
5. Create `app.bicep` with the correct Radius extensions and resource declarations.
6. Ask the user only for values that cannot be inferred safely.
...
- Infer the container port from explicit workload configuration such as `EXPOSE`, `listen`, or server launch arguments instead of base-image defaults.
...
- Read `src/web/Dockerfile` and `src/web/nginx.conf` together to infer the frontend port.
...
- Use container port `3000` for customer-agent because `src/web/Dockerfile` exposes `3000` and `src/web/nginx.conf` listens on `3000`.
